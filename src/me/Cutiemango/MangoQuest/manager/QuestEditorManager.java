...
public class QuestEditorManager
{
	...
	public static void mainGUI(Player p)
	{
		...
		p1.add(new InteractiveText("&0&l- 《新增任務》").clickCommand("/mq e newquest").showText("&f點擊以新增任務")).changeLine();
		p1.changeLine();
		p1.add(new InteractiveText("&0&l- 《編輯任務》").clickCommand("/mq e edit").showText("&f點擊進入編輯任務介面")).changeLine();
		p1.changeLine();
		p1.add(new InteractiveText("&0&l- 《移除任務》").clickCommand("/mq e remove").showText("&f點擊進入移除任務介面")).changeLine();
		p1.changeLine();

		if (isInEditorMode(p) && QuestEditorManager.getCurrentEditingQuest(p).getInternalID() != null)
		{
			p1.add(new InteractiveText("&c&l- 《返回編輯》").clickCommand("/mq e gui").showText("&e您還有尚未儲存的任務！")).changeLine();
			p1.changeLine();
			p1.add(new InteractiveText("&c&l- 《退出編輯》").clickCommand("/mq e exit").showText("&c&l所做的任何變更都不會儲存。")).changeLine();
		}
	...
	public static void editQuest(Player p)
	{
		...
		p2.add(new InteractiveText("[前往編輯頁面]").clickCommand("/mq e edit req").showText("&f點擊以前往 &3任務需求&f編輯頁面")).changeLine();
		p2.add("&0任務&5&l事件： ").changeLine();
		p2.add(new InteractiveText("[前往編輯頁面]").clickCommand("/mq e edit evt").showText("&f點擊以前往 &5任務事件&f編輯頁面")).changeLine();
		p2.add("&0任務&c&l階段： ").changeLine();
		p2.add(new InteractiveText("[前往編輯頁面]").clickCommand("/mq e edit stage").showText("&f點擊以前往 &c任務階段&f編輯頁面")).changeLine();
	...
	public static void editQuestStages(Player p)
	{
		...
		{
			p1.addExtra(TextComponentFactory.regClickCmdEvent(QuestUtil.translateColor("[編輯階段(" + i + ")]"), "/mq e edit stage " + i));
			p1.addExtra(TextComponentFactory.regClickCmdEvent(QuestUtil.translateColor("&c [移除]"), "/mq e remove stage " + i));
			p1.addExtra("\n");
		}
		p1.addExtra(TextComponentFactory.regClickCmdEvent(QuestUtil.translateColor("&c&l[新增]"), "/mq e addnew stage"));
		p1.addExtra("\n");
	...
	public static void editQuestObjects(Player p, int stage)
	{
		...
		{
			p1.addExtra(TextComponentFactory.regClickCmdEvent(QuestUtil.translateColor("[編輯目標(" + i + ")]"), "/mq e edit object " + stage + " " + i));
			p1.addExtra(TextComponentFactory.regClickCmdEvent(QuestUtil.translateColor("&c [移除]"), "/mq e remove object " + stage + " " + i));
			p1.addExtra("\n");

		}
		p1.addExtra(TextComponentFactory.regClickCmdEvent(QuestUtil.translateColor("&c&l[新增]"), "/mq e addnew object " + stage));
		p1.addExtra("\n");
	...
	public static void editQuestObject(Player p, int stage, int obj)
	{
		...
		switch (o.getConfigString())
		{
			...
			case "REACH_LOCATION":
				...
				p1.addExtra(QuestUtil.translateColor("(" + ((QuestObjectReachLocation) o).getLocation().getBlockX() + ", "
