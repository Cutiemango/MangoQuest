...
//				TextComponentFactory.registerClickCommandEvent(QuestUtil.translateColor("&0&l- 《新建任務》"), "/mqe newquest"), "&f點擊進入新增任務介面"));
		...
		p1.addExtra(TextComponentFactory.regHoverEvent(
				TextComponentFactory.regClickCmdEvent(QuestUtil.translateColor("&0&l- 《編輯任務》"), "/mqe edit"), "&f點擊進入編輯任務介面"));
		p1.addExtra("\n\n");
		p1.addExtra(TextComponentFactory.regHoverEvent(
				TextComponentFactory.regClickCmdEvent(QuestUtil.translateColor("&0&l- 《移除任務》"), "/mqe remove"), "&f點擊進入移除任務介面"));
		p1.addExtra("\n");
	...
	public static void removeConfirmGUI(Player p, Quest q){
		...
		p1.addExtra(QuestUtil.translateColor("\n&8&l(不可回復，請審慎考慮)"));
		p1.addExtra("\n\n  ");
		p1.addExtra(TextComponentFactory.regClickCmdEvent("&2&l【確定】", "/mqe remove quest " + q.getInternalID()));
		p1.addExtra(QuestUtil.translateColor(" &8&l/ "));
		p1.addExtra(TextComponentFactory.regClickCmdEvent("&c&l【取消】", "/mqe"));
		QuestGUIManager.openBook(p, p1);
	}
	...
	public static void editQuest(Player p){
		...
		p1.addExtra("\n");
		p1.addExtra(TextComponentFactory.regHoverEvent(
				TextComponentFactory.regClickCmdEvent("&6&l任務名稱", "/mqe edit name"), "&f點擊以編輯 任務名稱"));
		p1.addExtra(QuestUtil.translateColor("： &0" + q.getQuestName()));
		p1.addExtra("\n");
		p1.addExtra(TextComponentFactory.regHoverEvent(
				TextComponentFactory.regClickCmdEvent("&0&l任務NPC ", "/mqe edit npc"), "&f點擊以編輯 任務NPC"));
		...
		p1.addExtra(TextComponentFactory.regClickCmdEvent("&7[編輯]", "/mqe edit req"));
		p1.addExtra("\n");
		p1.addExtra(QuestUtil.translateColor("&0&l任務事件： "));
		p1.addExtra(TextComponentFactory.regClickCmdEvent("&7[編輯]", "/mqe edit evt"));
		p1.addExtra("\n");
		p1.addExtra(TextComponentFactory.regHoverEvent(
				TextComponentFactory.regClickCmdEvent("&0&l重複執行", "/mqe edit redo"), "&f點擊以編輯 是否可重複執行"));
		if (q.isRedoable()){
			p1.addExtra(QuestUtil.translateColor("&0&l： &a是"));
			p1.addExtra("\n");
			p1.addExtra(TextComponentFactory.regHoverEvent(
					TextComponentFactory.regClickCmdEvent("&0&l等待時間", "/mqe edit redodelay"), "&f點擊以編輯 重複執行等待時間"));
		...
		p1.addExtra("\n");
		p1.addExtra(TextComponentFactory.regHoverEvent(
				TextComponentFactory.regClickCmdEvent("&2&l【同步伺服器與設定檔】", "/mqe sa"), "&c&l注意： 目前持有此任務的玩家都會遺失任務進度。"));
		p1.addExtra("\n");
		p1.addExtra(TextComponentFactory.regHoverEvent(
				TextComponentFactory.regClickCmdEvent("&5&l【同步伺服器】", "/mqe sl"), "&c&l注意： 目前持有此任務的玩家都會遺失任務進度。"));
		p1.addExtra("\n");
		p1.addExtra(TextComponentFactory.regHoverEvent(
				TextComponentFactory.regClickCmdEvent("&9&l【同步設定檔】", "/mqe sc"), "&c&l注意： 目前持有此任務的玩家都會遺失任務進度。"));
		
		p2.addExtra("\n");
		p2.addExtra(TextComponentFactory.regClickCmdEvent("&7[編輯]", "/mqe edit outline"));
	...
	public static void editQuestTrigger(Player p){
		...
		for (QuestTrigger qt : q.getTriggers()){
			...
			if (qt.getType().equals(TriggerType.TRIGGER_STAGE_START)
				...
				p1.addExtra(TextComponentFactory.regClickCmdEvent("&7[編輯]", "/mqe edit evt " + index + " "
			...
			p1.addExtra(TextComponentFactory.regClickCmdEvent("&7[移除]", "/mqe remove evt " + index));
			p1.addExtra("\n");
			index++;
		}
		p1.addExtra(TextComponentFactory.regClickCmdEvent("&0&l[新增]", "/mqe addnew evt"));
		p1.addExtra("\n");
		p1.addExtra(TextComponentFactory.regClickCmdEvent("&0&l[返回任務選單]", "/mqe gui"));
		QuestGUIManager.openBook(p, p1);
	}
		...
		for (RequirementType t : RequirementType.values()){
			...
			case ITEM:
				...
				for (ItemStack item : (List<ItemStack>)q.getRequirements().get(t)){
					...
					p1.addExtra(TextComponentFactory.regClickCmdEvent("&7[編輯]", "/mqe edit req ITEM " + i));
					p1.addExtra(TextComponentFactory.regClickCmdEvent("&7[移除]", "/mqe remove req ITEM " + i));
					p1.addExtra("\n");
					i++;
				}
				p1.addExtra(TextComponentFactory.regClickCmdEvent("&0&l[新增]", "/mqe addnew req ITEM " + i));
				p1.addExtra("\n");
				break;
			case LEVEL:
				p1.addExtra("等級需求： " + q.getRequirements().get(t).toString() + " ");
				p1.addExtra(TextComponentFactory.regClickCmdEvent("&7[編輯]", "/mqe edit req LEVEL"));
				p1.addExtra("\n");
				break;
			case MONEY:
				p1.addExtra("金錢需求： " + q.getRequirements().get(t).toString() + " ");
				p1.addExtra(TextComponentFactory.regClickCmdEvent("&7[編輯]", "/mqe edit req MONEY"));
				p1.addExtra("\n");
				break;
			...
			case NBTTAG:
				...
				for (String s : (List<String>)q.getRequirements().get(t)){
					p2.addExtra("- ");
					p2.addExtra(s);
					p2.addExtra(TextComponentFactory.regClickCmdEvent("&7[編輯]", "/mqe edit req NBTTAG " + i));
					p2.addExtra(TextComponentFactory.regClickCmdEvent("&7[移除]", "/mqe remove req NBTTAG " + i));
					p2.addExtra("\n");
					i++;
				}
				p2.addExtra(TextComponentFactory.regClickCmdEvent("&0&l[新增]", "/mqe addnew req NBTTAG " + i));
				p2.addExtra("\n");
				break;
			case QUEST:
				p2.addExtra("任務需求：");
				...
				for (String s : (List<String>)q.getRequirements().get(t)){
					p2.addExtra("- ");
					p2.addExtra(s);
					p2.addExtra(TextComponentFactory.regClickCmdEvent("&7[編輯]", "/mqe edit req QUEST " + i));
					p2.addExtra(TextComponentFactory.regClickCmdEvent("&7[移除]", "/mqe remove req QUEST " + i));
					p2.addExtra("\n");
					i++;
				}
				p2.addExtra(TextComponentFactory.regClickCmdEvent("&0&l[新增]", "/mqe addnew req QUEST " + i));
				p2.addExtra("\n");
				break;
			case SCOREBOARD:
				p2.addExtra("記分板需求：");
				...
				for (String s : (List<String>)q.getRequirements().get(t)){
					p2.addExtra("- ");
					p2.addExtra(s);
					p2.addExtra(TextComponentFactory.regClickCmdEvent("&7[編輯]", "/mqe edit req SCOREBOARD " + i));
					p2.addExtra(TextComponentFactory.regClickCmdEvent("&7[移除]", "/mqe remove req SCOREBOARD " + i));
					p2.addExtra("\n");
					i++;
				}
				p2.addExtra(TextComponentFactory.regClickCmdEvent("&0&l[新增]", "/mqe addnew req SCOREBOARD " + i));
				p2.addExtra("\n");
				break;
			}
		}
	...
	public static void selectTriggerType(Player p){
		...
		for (TriggerType t : TriggerType.values()){
			p1.addExtra(TextComponentFactory.regClickCmdEvent("- [" + t.toCustomString() + "]", "/mqe addnew evt " + t.toString()));
			p1.addExtra("\n");
		}
		QuestGUIManager.openBook(p, p1);
	}
	...
	public static void selectStage(Player p, TriggerType t){
		...
		for (int s = 1; s <= q.getStages().size(); s++){
			p1.addExtra(TextComponentFactory.regClickCmdEvent("- [階段" + s + "]", "/mqe addnew evt " + t.toString() + " " + s));
			p1.addExtra("\n");
		}
		QuestGUIManager.openBook(p, p1);
	}
	...
	public static void selectTriggerObject(Player p, TriggerType t, int s){
		...
		for (TriggerObject o : TriggerObject.values()){
			if (t.equals(TriggerType.TRIGGER_STAGE_START) || t.equals(TriggerType.TRIGGER_STAGE_FINISH)){
				p1.addExtra(TextComponentFactory.regClickCmdEvent("- [" + o.toCustomString() + "]", "/mqe addnew evt " + t.toString() + " " + s + " " + o.toString()));
				p1.addExtra("\n");
				continue;
			}
			p1.addExtra(TextComponentFactory.regClickCmdEvent("- [" + o.toCustomString() + "]", "/mqe addnew evt " + t.toString() + " " + o.toString()));
			p1.addExtra("\n");
		}
		QuestGUIManager.openBook(p, p1);
	}
