...
public class DatabaseSaver
{
	...
	public static void saveQuestPlayerData(QuestPlayerData pd) {
		int PDID = pd.getPDID();
		savePlayer(pd.getPlayer());
	...
	private static void saveFriendPoint(int NPCID, int friendPoint, int PDID) {
		Connection conn = DatabaseManager.getConnection();
		try {
		...
		try {
			...
			if(!select.executeQuery().next()) {
				PreparedStatement insert = conn.prepareStatement(
						"INSERT INTO mq_finishedconv (PDID, ConvID) values (?, ?)");
				insert.setInt(1, PDID);
	...
	private static void saveQuestProgress(QuestProgress questProgress, int PDID) {
		...
		try {
			...
			if(select.executeQuery().next()) {
				...
				update.setLong(3, questProgress.getQuest().getVersion().getVersion());
				...
				insert.setLong(6, questProgress.getQuest().getVersion().getVersion());
				insert.execute();
			}
		} catch(SQLException e) {
			e.printStackTrace();
	...
	private static void removeFinishedQuests(Set<QuestProgress> progresses, int PDID) {
		...
		try {
			...
			while(results.next()) {
				String QID = results.getString("QuestID");
				if(progresses.stream().noneMatch(questProgress -> questProgress.getQuest().getInternalID().equals(QID))) {
					PreparedStatement delete = conn.prepareStatement("DELETE FROM mq_questprogress WHERE PDID = ? AND QuestID = ?");
					delete.setInt(1, PDID);
					delete.setNString(2, QID);
					delete.execute();
				}
			}
		} catch(SQLException e) {
	...
	private static void saveFinishedQuest(QuestFinishData questData, int PDID) {
		...
		try {
			...
			if(select.executeQuery().next()) {
				PreparedStatement update = conn.prepareStatement(
						"UPDATE mq_finishedquest set FinishedTimes = ?, RewardTaken = ?, TakeStamp = ? WHERE PDID = ? AND QuestID = ?");
				...
				PreparedStatement insert = conn.prepareStatement(
						"INSERT INTO mq_finishedquest (PDID, QuestID, TakeStamp, FinishedTimes, RewardTaken) values (?, ?, ?, ?, ?)");
				insert.setInt(1, PDID);
