...
public class ConversationEditorManager
{
	...
	public static void editConversation(Player p) {
		...
		book.add(new InteractiveText(I18n.locMsg("ConversationEditor.ConvNPC")).showText(I18n.locMsg("ConversationEditor.ConvNPC.ShowText"))
		...
		// Speical conv
		book.createNewPage();
		book.add(new InteractiveText(I18n.locMsg("ConversationEditor.ConvType")).showText(I18n.locMsg("ConversationEditor.ConvType.ShowText"))
		...
		if (conv instanceof FriendConversation) {
			...
			book.add(new InteractiveText(I18n.locMsg("ConversationEditor.QuestFullMessage")).clickCommand("/mq ce edit fullmsg")
					.showText(I18n.locMsg("ConversationEditor.QuestFullMessage.ShowText"))).changeLine();
			book.add(sconv.getQuestFullMessage()).changeLine();

			book.add(new InteractiveText(I18n.locMsg("ConversationEditor.QuestAcceptMessage")).clickCommand("/mq ce edit acceptmsg")
					.showText(I18n.locMsg("ConversationEditor.QuestAcceptMessage.ShowText")));
			book.add(sconv.getAcceptMessage()).changeLine();

			book.add(new InteractiveText(I18n.locMsg("ConversationEditor.QuestDenyMessage")).clickCommand("/mq ce edit denymsg")
					.showText(I18n.locMsg("ConversationEditor.QuestDenyMessage.ShowText")));
			...
			for (QuestBaseAction act : ((StartTriggerConversation) conv).getAcceptActions()) {
				...
				if (EnumAction.NO_OBJ_ACTIONS.contains(act.getActionType()))
					...
					book.add(new InteractiveText(act.getActionType().toCustomString()).showText(
							I18n.locMsg("ConversationEditor.ActionObject") + QuestChatManager
									.toNormalDisplay(I18n.locMsg("QuestJourney.NPCFriendMessage", npc.getName(), split[0]))));
				} else
					book.add(new InteractiveText(act.getActionType().toCustomString())
							.showText(I18n.locMsg("ConversationEditor.ActionObject") + act.getObject()));
		...
		book.add(
				new InteractiveText(I18n.locMsg("QuestEditor.SyncSevAndLoc")).clickCommand("/mq ce sa").showText(I18n.locMsg("QuestEditor.WarnSave")))
				.changeLine();
		book.add(new InteractiveText(I18n.locMsg("QuestEditor.SyncSev")).clickCommand("/mq ce sl").showText(I18n.locMsg("QuestEditor.WarnSave")))
				.changeLine();
		book.add(new InteractiveText(I18n.locMsg("QuestEditor.SyncLoc")).clickCommand("/mq ce sc").showText(I18n.locMsg("QuestEditor.WarnSave")))
				.changeLine();
		book.changeLine();
		book.add(new InteractiveText(I18n.locMsg("QuestEditor.QuitEditor")).clickCommand("/mq ce exit")
				.showText(I18n.locMsg("QuestEditor.ExitEditor.ShowText"))).changeLine();
	...
	public static void createConversation(Player p) {
		...
		InteractiveText npc = (qc.getNPC() != null) ?
				new InteractiveText("").showNPCInfo(qc.getNPC()) :
				new InteractiveText(I18n.locMsg("QuestEditor.NotSet"));
	...
	public static void selectActionType(Player p, String type, String mode, int index) {
		...
		for (EnumAction act : EnumAction.values()) {
			p1.add(new InteractiveText("- [" + act.toCustomString() + "]")
					.clickCommand("/mq ce " + mode + " " + type + " " + index + " " + act.toString()));
			p1.changeLine();
		}
		QuestBookGUIManager.openBook(p, p1);
	}
