		...
		if (args.length == 4)
			...
			if (act == EnumAction.NPC_TALK) {
				EditorListenerHandler.register(sender,
						new EditorListenerObject(ListeningType.STRING, "mq ce new " + acttype + " " + index + " " + act.toString(),
								Syntax.of("S@I", I18n.locMsg("Syntax.NPCTalk"), "@")));
				QuestBookGUIManager.openInfo(sender, I18n.locMsg("EditorMessage.NPCTalk"));
				return;
			}
			EditorListenerHandler.register(sender,
					new EditorListenerObject(ListeningType.STRING, "mq ce new " + acttype + " " + index + " " + act.toString(), null));
