...
public class QuestEditorCommand implements CommandExecutor {
	...
	public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
		...
		if (cmd.getName().equals("mqe")) {
			if (args.length == 0){
				QuestEditorManager.mainGUI(p);
				return false;
			}
			if (args.length == 1) {
				if (args[0].equalsIgnoreCase("edit")){
					QuestEditorManager.editGUI(p);
					return false;
				}
				else if (args[0].equalsIgnoreCase("remove")){
					QuestEditorManager.removeGUI(p);
					return false;
				}
					...
					case "confirm":
						if (args.length == 3){
							if (QuestUtil.getQuest(args[2]) != null){
								Quest target = QuestUtil.getQuest(args[2]);
								QuestEditorManager.removeConfirmGUI(p, target);
								return false;
							}
						}
					case "req":
